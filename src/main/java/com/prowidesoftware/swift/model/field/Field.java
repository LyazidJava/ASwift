...
public abstract class Field implements PatternContainer {
    ...
    /**
     * Creates a new field and initializes its components with content from the parameter value. The value is parsed with
     * {@link #parse(String)}
     *
    ...
    /**
     * Parses the parameter value into the internal components structure. Used to update all components from a full new value, as an
     * alternative to setting individual components. Previous components value is overwritten.
     * <br>
    ...
    /**
     * Get a localized, suitable for showing to humans string of the field values. This method is overwritten when necessary by
     * subclasses.
     *
    ...
    /**
     * Get the given component as the given object type. If the class is not recognized, it returns null, as well as if conversion
     * fails.
     *
    ...
    /**
     * Get the given component as a number object This method internall y calls {@link #getComponentAs(int, Class)}, and casts the
     * result
     *
     * @since 7.8
     */
     ...
     * @param skipLast if true the last component will not be included in the join, and where the "last" component is understood as
     * the last not empty component (this is not necessary the last component of the field's component list.
     *
     * @return s
     */
    ...
    /**
     * Finds the first component starting with the given codeword between slashes, and returns the component subvalue. For example,
     * for the following field value<br />
    ...
    /**
     * Creates a Field instance for the given Tag object, using reflection. The created object is populated with parsed components
     * data from the Tag.
     *
    ...
    /**
     * @deprecated field labels varies depending on the specific MT and sequence, label should be retrieve using
     * {@link #getLabel(String, String, String, Locale)} with proper MT and sequence identifiers
     */
    @Deprecated
    @DeleteSchedule(2018)
    ...
    /**
     * Returns the field business description name, using resource bundle from pw_swift_labels property files. Field names may be
     * generic for all usages, or may differ for particular letter option, message type or even sequence of a message type. The
     * property supports all this kind of definitions with generic labels and specific ones. The following example illustrate the
     * precedence of bundle keys that are checked for field 50:<br />
     * <ul>
     * <li>50K[103][B]</li>
     * <li>50a[103][B]</li>
     * <li>50K[103]</li>
     ...
     * </ul>
     *
     * @param fieldName field name of the field to retrieve its label, if the combination of number and letter option is provided
     * then a specific label is returned; is the letter option is omitted then a more generic label is returned.
     ...
     * @return <code>true</code> if this field names equals one in the list of names and <code>false</code> in other case
    ...
    /**
     * Compare the value of the component1 of this field with <code>compare</code> Same as <code>is(1, compare)</code>
     */
    ...
    /**
     * Compare the value of the component1 of this field with <code>compare1</code> and the value of component2 with
     * <code>compare2</code>
     */
     ...
     * This is not the amount of components present in the field instance, but the total amount of components that this field
     * accepts as defined.
     */
     ...
     *
     * Notice that line instance numbers are static and relevant to the field components definition, and not relative to the
     * particular instance value. For example field 35B Line[1] will be the line with the ISIN number, regardless of the ISIN number
     * present or not in the particular field instance. If that ISIN line is not present in the parameter field, the method will
     * return null.<br><br>
     *
     * Also notice that a line may be composed by several components, there is no linear relation between component numbers and
     * lines numbers.<br>
     *
    ...
    /**
     * Returns true if the value only contains '/' characters (one or many but only that character)
     */
     ...
     *
     * It also can remove meaningless component separators (If the resulting string only contains the component separator "/", or
     * starts with ":" or starts with "/" separators all of them will also be removed).
     *
